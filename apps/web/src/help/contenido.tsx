/**
 * Contenido del Centro de ayuda, separado de la vista que lo pinta.
 *
 * Son ~700 líneas de texto que antes vivían dentro del componente. Sacarlas tiene dos efectos: al
 * escribir un artículo ya no se toca el render, y —lo que de verdad importaba— el índice pasa a ser
 * un **dato consultable**, así que el asistente puede citar el artículo exacto en vez de decir
 * genéricamente «está en el Centro de ayuda».
 *
 * Esta es documentación **conceptual**, no un registro de cambios: no se deriva del CHANGELOG (de
 * ahí salen Novedades y el historial del asistente) ni de `docs/`, que es documentación técnica
 * para el asistente. Son tres registros con público distinto, y mezclarlos empeoraría los tres.
 */
import type React from 'react';

export type Section = 'inicio' | 'manual' | 'kb' | 'faq' | 'glosario';
...
export const GLOSARIO: Array<[string, string, string]> = [
  [
    'Trades',
    'básico',
...
export const RESUMEN: Record<string, string> = {
  ...
  'Análisis fundamental: el funding, y solo contra las compras':
    'Por qué el funding solo desaconseja comprar, y por qué todavía no manda.',
  'Cuando hay varias señales a la vez: ¿son varias oportunidades?':
    'Correlación entre activos: por qué tres señales pueden ser una sola apuesta.',
...
export const RUTAS: Array<{ icono: string; titulo: string; sub: string; sec: Section; art: string }> = [
  {
    icono: '🧭',
...
/** Todos los artículos, en el orden en que se muestran. Es el índice que consulta el asistente. */
export const ARTICULOS: Article[] = [...MANUAL, ...KB, ...KB_EXTRA, ...FAQ];

/**
 * Busca el artículo que mejor responde a un término.
 *
 * Sirve para que el asistente remita al sitio exacto. Si algún día se renombra un artículo que él
 * citaba, esto devuelve `null` y el test de coherencia lo caza — en vez de dejar una remisión rota
 * que nadie nota hasta que alguien la sigue.
 */
export function buscarArticulo(termino: string): Article | null {
  const t = termino.trim().toLowerCase();
  if (!t) return null;
  const enTitulo = ARTICULOS.find((a) => a.title.toLowerCase().includes(t));
  if (enTitulo) return enTitulo;
  return ARTICULOS.find((a) => (RESUMEN[a.title] ?? '').toLowerCase().includes(t)) ?? null;
}
